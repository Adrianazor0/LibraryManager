 ...
 * Upgraded with Unicode Accent Normalization, Whole-Text Search, Subject Intents & Anaphora Memory
 */
export const askLibraryBot = async (req: Request, res: Response) => {
    try {
        ...
        // 2. DETECCIÓN DE ENTIDADES, ASIGNATURAS Y GRADOS ACADÉMICOS
        ...
        // Detección de Asignaturas Específicas
        const isFisica = q.includes("física") || q.includes("fisica") || q.includes("movimiento");
        const isMatematicas = q.includes("matemática") || q.includes("matematica") || q.includes("álgebra") || q.includes("algebra") || q.includes("geometría") || q.includes("geometria");
        const isHistoria = q.includes("historia") || q.includes("española") || q.includes("espanola") || q.includes("moya pons");
        const isBiologia = q.includes("biología") || q.includes("biologia") || q.includes("química") || q.includes("quimica") || q.includes("ciencias");
        const isCuentos = q.includes("cuento") || q.includes("cuentos") || q.includes("novela") || q.includes("literatura");

        // Detección de Obras / Autores Específicos
        ...
        const isBaldor = q.includes("baldor");
        ...
        // CASO 0: Recomendación por Grado Académico (3ro de Secundaria)
        if (isAskingAboutGrade3 && isAskingRecommendation) {
            const grade3Books = allBooks.filter(b => {
                const t = (b.title + " " + b.author).toLowerCase();
                return t.includes("quiroga") || t.includes("locura y de muerte") || t.includes("geometría") || t.includes("geometria") || t.includes("española en el siglo xvi") || t.includes("espanola en el siglo xvi") || t.includes("tippens") || t.includes("física general");
            });
        ...
        // CASO 1: Recomendación por Grado Académico (4to de Secundaria)
        ...
        // CASO 2: Recomendación por Grado Académico (5to de Secundaria)
        ...
        // CASO 3: Recomendación por Grado Académico (6to de Secundaria)
        ...
        // CASO 4: Consulta Específica sobre Libros de Física
        else if (isFisica) {
            const fisBooks = allBooks.filter(b => {
                const text = (b.title + " " + b.author + " " + (b.category || '')).toLowerCase();
                return text.includes("física") || text.includes("fisica") || text.includes("tippens") || text.includes("sears");
            });
            reply = `⚡ **Recursos de Física Disponibles en Catálogo**:\n\nContamos con **${fisBooks.length} texto(s) de Física** para la formación científica en el Liceo La Ureña:`;
            suggestedBooks = fisBooks;
        }
        // CASO 5: Consulta Específica sobre Matemáticas / Geometría / Álgebra
        else if (isMatematicas || isBaldor) {
            const mathBooks = allBooks.filter(b => {
                const text = (b.title + " " + b.author + " " + (b.category || '')).toLowerCase();
                return text.includes("matemática") || text.includes("matematica") || text.includes("álgebra") || text.includes("algebra") || text.includes("geometría") || text.includes("geometria") || text.includes("baldor");
            });
            reply = `📐 **Recursos de Matemáticas y Geometría en Catálogo**:\n\nContamos con **${mathBooks.length} texto(s) clave de Matemáticas** en el catálogo:`;
            suggestedBooks = mathBooks;
        }
        // CASO 6: Consulta sobre El Señor de los Anillos / Tolkien
        ...
        // CASO 7: Consulta Exclusiva sobre Harry Potter
            ...
            if (hpBooks.length > 0) {
                ...
                reply = `❌ **Recurso No Disponible en Catálogo**:\n\nActualmente **NO disponemos** de libros de la saga **"Harry Potter"** en el inventario activo de la biblioteca del Liceo Vespertino La Ureña.`;
                suggestedBooks = [];
            }
        }
        // CASO 8: Consulta Exclusiva sobre Pinocho
        else if (isPinocho) {
            ...
            if (pinochoBooks.length > 0) {
                reply = `🧸 **Stock de Pinocho**:\n\nContamos con **${pinochoBooks[0].stockAvailable} ejemplares disponibles** de **"${pinochoBooks[0].title}"** de ${pinochoBooks[0].author} en la sección de ${pinochoBooks[0].section}.`;
                suggestedBooks = pinochoBooks;
            }
        }
        // CASO 9: Consulta Exclusiva sobre Diccionario de Dominicanismos
        ...
        // CASO 10: Pregunta de seguimiento sobre tiempo de préstamo de libro específico
        ...
        // CASO 11: Saludos puros
        else if (q === "hola" || q === "buenas" || q === "saludos" || q === "hola bibliobot") {
            reply = "¡Hola! 👋 Soy BiblioBot, tu Asistente Conversacional Inteligente 24/7 del Liceo Vespertino La Ureña. ¿En qué te puedo colaborar hoy? Puedes preguntarme por recomendaciones de 3ro, 4to, 5to o 6to de secundaria, libros de física o normativas de préstamo.";
        }
        // CASO 12: Búsqueda Semántica RAG Estricta con Normalización de Acentos
        else {
            ...
            // Función auxiliar para normalizar acentos para la comparación (ej: "física" -> "fisica")
            const normalize = (str: string) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();

            const rawTokens = q.split(/\s+/).filter((t: string) => t.length > 2 && !stopWords.includes(t));
            const normTokens = rawTokens.map(normalize);

            let matched = allBooks.filter(book => {
                const fullText = (book.title + " " + book.author + " " + (book.category || '') + " " + (book.description || '')).toLowerCase();
                const normFullText = normalize(fullText);

                if (normTokens.length === 0) return false;

                return normTokens.some((token: string) => normFullText.includes(token));
            });
